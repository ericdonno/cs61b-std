...
目前方案不是在否定方向 A/B，而是在它们的基础上**做减法**——砍掉 Boss 的重度系统、砍掉精英怪的模糊定位，只保留一个核心改动：**让每个普通敌人都有存在感**。这恰好也是 [历史 Agent 构建指南](legacy-agent-build-guide.md) 的 Phase 1~6 所做的事——RemoteBrain 替代 RuleBasedBrain，GameStateSnapshot 扩展，记忆系统。只需要在 Phase 1 和 Phase 3 中额外加入视线阻挡算法即可。
