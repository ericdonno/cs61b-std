...
> 本文档是 LLMBrain 的分阶段实施手册。和原 [DungeonMind_Build_Guide.md](DungeonMind_Build_Guide.md) 不同，**这里不会用纯 Java 实现 LLM 大脑**。我们将把 AI 层独立成一个 Python Agent Runtime，Java 只负责游戏模拟。读完本文后，你将建成一个：**Java 游戏引擎 + Python LangGraph Agent** 的混合架构，敌人通过 Tool Calling 与游戏世界交互。
>
