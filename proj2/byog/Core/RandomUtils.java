...
public class RandomUtils {
    ...
    /**
     * @param bias >1偏向a，<1偏向b，=1为均匀分布
     * */
    public static double biasUniform(Random random, double a, double b, double bias) {
        if (!(a < b)) {
            throw new IllegalArgumentException("invalid range: [" + a + ", " + b + ")");
        }
        if (bias <= 0) {
            throw new IllegalArgumentException("bias must be positive: " + bias);
        }
        return a + Math.pow(uniform(random), bias) * (b - a);
    }
